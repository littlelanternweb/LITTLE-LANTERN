...
export function AdminSidebar({ session }: { session: any }) {
  ...
  return (
    <aside className="w-64 bg-white border-r border-[#F5F5F4] text-[#57534E] flex-shrink-0 hidden md:flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-10 relative">
      <div className="p-6 border-b border-[#F5F5F4]">
        <Link href="/admin" className="flex items-center gap-3 text-[#1C1917] group">
          <div className="relative h-10 w-10 overflow-hidden rounded-lg shadow-sm border border-[#F5F5F4] group-hover:shadow-md transition-shadow">
            <Image src="/logo.jpg" alt="Little Lantern Logo" fill className="object-cover" />
          </div>
          <div className="flex flex-col">
            <span className="font-display font-semibold text-lg tracking-tight leading-tight">Little Lantern</span>
            <span className="text-[11px] font-medium text-[#047857] tracking-wider uppercase">Workspace</span>
          </div>
        </Link>
      </div>
      
      <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto scrollbar-hide">
        <p className="px-3 text-xs font-semibold text-[#A8A29E] uppercase tracking-wider mb-4">Main Menu</p>
        {links.map((link) => {
          ...
          return (
            ...
            >
              ...
              <Icon className={cn("w-4 h-4 transition-colors", isActive ? "text-[#047857]" : "text-[#A8A29E] group-hover:text-[#1C1917]")} /> 
              {link.name}
            </Link>
          );
        })}
      ...
      <div className="p-4 border-t border-[#F5F5F4] bg-white">
        <div className="flex items-center gap-3 px-3 py-3 mb-2 rounded-xl bg-[#FCFBF9] border border-[#F5F5F4]">
          <div className="w-10 h-10 rounded-full bg-[#E7E5E4] border border-[#D6D3D1] flex items-center justify-center text-[#1C1917] font-semibold text-lg shrink-0">
            {session.user?.name?.charAt(0)}
          </div>
          <div className="min-w-0 flex-1">
            <p className="font-medium text-[#1C1917] text-sm truncate">{session.user?.name}</p>
            <p className="text-[11px] uppercase tracking-wider text-[#047857] font-semibold truncate">{role}</p>
          </div>
        </div>
