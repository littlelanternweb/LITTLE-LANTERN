...
export function JobsClient({ initialApplications }: { initialApplications: any[] }) {
  ...
  const [teacherSyllabusFilter, setTeacherSyllabusFilter] = useState("ALL");
  const [teacherStandardFilter, setTeacherStandardFilter] = useState("ALL");
  ...
  const filteredApps = applications.filter(app => {
    ...
    if (positionFilter === "Teacher" && app.position === "Teacher" && app.metadata) {
      ...
      if (teacherSyllabusFilter !== "ALL" && !app.metadata.syllabus?.includes(teacherSyllabusFilter)) {
        return false;
      }
      if (teacherStandardFilter !== "ALL" && !app.metadata.classes?.includes(teacherStandardFilter)) {
        return false;
      }
    }
  ...
  return (
    <Card className="rounded-2xl border-stone-200/60 shadow-sm overflow-hidden">
      <CardHeader className="bg-stone-50/50 border-b border-stone-100 py-4 px-6 flex flex-col justify-between gap-4">
        ...
        {positionFilter === "Teacher" && (
          <div className="flex flex-wrap items-center gap-2 pt-2 mt-2 border-t border-slate-200/60">
            ...
            <input 
              ...
              className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm bg-white min-w-[150px]"
            />
            <select 
              value={teacherSyllabusFilter} 
              onChange={e => setTeacherSyllabusFilter(e.target.value)}
              className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm bg-white"
            >
            ...
            <select 
              value={teacherStandardFilter} 
              onChange={e => setTeacherStandardFilter(e.target.value)}
              className="px-3 py-1.5 border border-slate-200 rounded-lg text-sm bg-white"
            >
              <option value="ALL">All Standards</option>
              {CLASSES_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
        )}
      </CardHeader>
